# X2D / 907 WiFi 地区设置（开源本地版）

中文 | **[English README](README_EN.md)**

> **本工具完全免费开源，任何收费行为皆为倒卖。** 唯一开源地址：https://github.com/WeiCheng97/Hasselblad-X2d-series-5g-unlock

通过相机 WiFi 热点直连哈苏 X2D / X2D II / 907X & CFV 100C 的工厂诊断通道，读取/修改相机的 **WiFi 地区**（wifiRegion），用于解除日版等地区的 **5GHz WiFi 限制**，并可随时恢复。
...
到 [**Releases 页面**](https://github.com/WeiCheng97/Hasselblad-X2d-series-5g-unlock/releases/latest) 下载（成品不放在源码仓库里）：
...
通道（X2D v4.2.0 与 X2D II v1.3.16.2 逐字节一致，固件逆向实证；CFV 100C v4.0.0 社区实测一致）：
...
- **支持机型**：Hasselblad X2D 100C（固件 v4.2.0 实测）、X2D II 100C（v1.3.16.2 实测）、907X & CFV 100C（v4.0.0，社区实测；热点名形如 `CFV 100C 008987`）。其它固件版本理论一致，未逐一实测。
