#!/bin/bash
# X2D WiFi 地区一键设置 —— 双击运行，任何 Mac 无需安装任何东西
# 原理：系统自带 Perl 通过相机热点（TCP 30303）调用原厂产测命令读写 WiFi 地区
# 前置：Mac 已连接相机热点（如 X2D II 100C 012343）；热点休眠时先用手机 Phocus 蓝牙唤醒
...
sub connect_fail_msg { my ($h) = @_;
  return "[x] 连不上相机 $h:30303（$!）\n" .
    "排查清单：\n" .
    "  1. Mac 是否已连接相机热点 WiFi？（系统设置 → Wi-Fi，如 X2D II 100C 012343）\n" .
...
while true; do
  echo ""
  echo "=============================================="
  echo "   X2D / X2D II WiFi 地区设置（一键工具）"
  echo "=============================================="
  echo "   1) 查看当前地区（只读，安全）"
